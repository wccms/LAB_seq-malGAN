...
class seqDiscriminator(nn.module):
    ...
    def forward(self, input, hidden0, cell0):
        ...
        ouput = self.embedding(input)   # batch_size * seq_len * embedding_dim
        ouput = ouput.permute(1, 0, 2)  # seq_len * batch_size * embedding_dim
        h0, c0 = self.init_hidden(input.size()[0])
        output, _ = self.LSTM(ouput, h0, c0)    # seq_len * batch_size * (hidden_dim*direction)
        if self.attnNet.size() > 0:
            attn_weight = output
            for i in range(self.attnNet.size()):
                attn_weight = self.attnNet(output)  # seq_len * batch_size * 1
